...
def dispNameAge():
    cur.execute("SELECT FIRST_NAME,MIDDLE_NAME,LAST_NAME,DOB FROM PYTHON")
    data = cur.fetchall()
    today = date.today()
    x = []
    for i in data:
...
def addRecord():
    ...
    insert = "INSERT INTO PYTHON VALUES('{}','{}','{}','{}','{}','{}','{}','{}')".format(prn, f_n, m_n, l_n, ad, ph,
                                                                                           em, dt)
    cur.execute(insert)
    mycon.commit()
...
def delRecord():
    ...
    query = "DELETE FROM PYTHON WHERE PRN='{}'".format(prn)
    cur.execute(query)
    mycon.commit()
...
def updatePhoneEmail():
    ...
    query = "UPDATE PYTHON SET MOBILE='{}',EMAIL='{}' WHERE PRN='{}'".format(mob, em, prn)
    cur.execute(query)
    mycon.commit()
...
def addCGPA(flag):
    if flag:
        query = "ALTER TABLE PYTHON ADD COLUMN CGPA FLOAT"
        cur.execute(query)
        mycon.commit()
        print("\n CGPA field added successfully...\n")
        cur.execute("SELECT PRN FROM PYTHON")
        x = cur.fetchall()
        for i in x:
            prn = i[0]
            print("Enter the CGPA of PRN ", prn, end=': ')
            cgpa = float(input())
            query = "UPDATE PYTHON SET CGPA='{}' WHERE PRN='{}'".format(cgpa, prn)
            cur.execute(query)
            mycon.commit()
        return False
    else:
...
def functions():
    print('''
            ...
            2. Display name and age of all PYTHON
            3. Add record
            4. Delete a record
...
if __name__ == "__main__":
    ...
    print("--" * 30)
    mycon = mysql.connector.connect(host="localhost", user="root", password="Adinath12", database="ADI")
    cur = mycon.cursor()
    global flag
    flag = False
    try:
        cur.execute("ALTER TABLE PYTHON DROP COLUMN CGPA")
    except:
        pass
    flag = True
